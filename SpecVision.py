...
class CondAns:
    ...
    def interactive_peak_fit_manual(self, exp_key, start_row=0, start_col=0):
        ...
        """
        from ipywidgets import FloatText, Dropdown, Label, HBox, VBox, Layout

            # ====== layout helpers ======
        field_layout = Layout(width='180px')
        label_style = {'description_width': '60px'}
        param_layout   = Layout(width='120px')   # for the main Amp/Center/Sigma fields
        minmax_layout  = Layout(width='100px')   # for the Min/Max fields
        ...
        row, col = start_row, start_col
        # list of tuples: (amp, amp_min, amp_max, cen, cen_min, cen_max, sig, sig_min, sig_max, gam, gam_min, gam_max, func, exists)
        peak_widgets = []
        ...
        def make_peak_row():
            ...
            def _on_func_change(change):
                if change['name'] == 'value':
                    cls = change['new']
                    # ExponentialGaussian requires 'gamma'
            ...
            row = HBox([amp_col, cen_col, sig_col, gam_col, func, exists], layout=row_layout)
            separator = HTML("<hr style='width:100%;margin:4px 0;color:lightgray;'>")

            return VBox([row, separator])
        ...
        def update_display():
            ...
            with out:
                ...
                for pfx in prefixes:
                    ...
                    # any extra params not in the ordered list
                    extras = sorted(
                        name.split('_', 1)[1]
                        for name in params.keys()
                        if name.startswith(pfx + '_') and name.split('_', 1)[1] not in suffix_order
        ...
        def on_key_change(change):
            if change['name'] == 'value' and change['new'] != change['old']:
                # reload all spectra for new dataset
                nonlocal data, wavelengths, image_scan, n_rows, n_cols, max_row, max_col
        ...
        peak_ctrl = HBox([btn_add_peak, btn_remove_peak, btn_fit_manual, threshold_entry, btn_fit_all],
    ...
    def interactive_peak_fit_manual_new(self, exp_key, start_row=0, start_col=0):
        """
        Interactive manual peak fitting with Prev/Next + direct-entry for row & col,
        plus controls to add/remove individual peak guesses (amplitude, center, sigma, function),
    ...
    def interactive_compare_conditions(self, start_row=0, start_col=0, ordered_conditions=None):
        """
        Interactive comparison of the same pixel across multiple experimental conditions,
        with ordering and mapping verification.
        """
        import numpy as np
        import matplotlib.pyplot as plt
        from ipywidgets import (
        ...
        if ordered_conditions:
            ...
            ordered_keys = keys_all

        # --------------------- widgets ------------------------
        w_row = BoundedIntText(value=start_row, min=0, max=n_rows - 1, description='Row:', layout=Layout(width='200px'))
        w_col = BoundedIntText(value=start_col, min=0, max=n_cols - 1, description='Col:', layout=Layout(width='200px'))

        btn_prev_row = Button(description='↑', layout=Layout(width='40px'))
        btn_next_row = Button(description='↓', layout=Layout(width='40px'))
        btn_prev_col = Button(description='←', layout=Layout(width='40px'))
        btn_next_col = Button(description='→', layout=Layout(width='40px'))

        # new: experiment order selector
        # w_order_sel = SelectMultiple(
        #     options=ordered_keys,
        #     value=tuple(ordered_keys),
        #     description='Conditions order:',
        ...
        w_show_all_maps = Checkbox(value=False, description='Show markers on all maps')

        out = Output(layout=Layout(border='1px solid #ccc', padding='5px'))
        ...
        def shift_row(d): w_row.value = np.clip(w_row.value + d, 0, n_rows-1)
        def shift_col(d): w_col.value = np.clip(w_col.value + d, 0, n_cols-1)

        # --------------------- plotting -----------------------
        def update_display(_=None):
            row, col = w_row.value, w_col.value
            # retrieve user‐specified order
            ...
            with out:
                out.clear_output(wait=True)
                # setup figure: waterfall + optional map series
                if w_show_all_maps.value:
                    fig, axes = plt.subplots(1, 1+N, figsize=(4*(1+N), 4), 
                                            gridspec_kw={'width_ratios':[2]+[1]*(N+1)})
                else:
                ...
                for idx, cond in enumerate(cond_order):
                    ...
                    wavelengths = self.data_dict[cond].get_wavelengths()
                    if wl is None:
                        wl = wavelengths
                    # map pixel
                    if cond == ref_key:
                        ...
                        if (row, col) in self.data_coordinates.get(cond, {}):
                            r, c = self.data_coordinates[cond][(row, col)]
                        else:
                            # skip condition if no mapping
                            continue

                    spec = data[r, c]
                    spec = _normalize(spec, w_norm.value)
                    specs.append(spec)
                    labels.append(cond)

                # compute offset
                off = w_offset.value
                if off == 0 and len(specs)>0:
                    off = 0.9 * np.median([np.max(s) for s in specs])

                # plot each condition
                for i, (cond, spec) in enumerate(zip(labels, specs)):
                    ax_wf.plot(wl, spec + i*off, color=colors[i], lw=w_lw.value, alpha=w_alpha.value, label=cond)
                ax_wf.set_xlabel('Wavelength')
                ax_wf.set_ylabel('Intensity + offset*k')
                if w_logy.value: ax_wf.set_yscale('log')
                ax_wf.legend(title='Condition', fontsize=8)
                ax_wf.set_title(f'Pixel ({row},{col}) across {len(labels)} conditions')

                # reference map (always show)
                ax0_map = axes[1] if not w_show_all_maps.value else axes[1]
                if image_ref is None:
                    ax0_map.text(0.5,0.5,'No map image', ha='center', va='center', fontsize=12)
                    ax0_map.axis('off')
                else:
                    ax0_map.imshow(image_ref, cmap='gray')
                    ax0_map.axis('off')
                    img_h, img_w = image_ref.shape[:2]
                    cell_w, cell_h = img_w / n_cols, img_h / n_rows
                    x0 = col*cell_w - cell_w/2
                    y0 = row*cell_h - cell_h/2
                    ax0_map.add_patch(Rectangle((x0,y0), cell_w, cell_h,
                                                linewidth=2, edgecolor='red', facecolor='none'))
                    ax0_map.set_title(f'Ref map: {ref_key}')

                # if show all maps, plot each condition’s map with its marker
                if w_show_all_maps.value:
                    for i, cond in enumerate(labels):
                        ax_map = axes[1 + i]
                        img = self.data_dict[cond].get_live_scan()
                        if img is None:
                            ax_map.text(0.5,0.5,'No map', ha='center', va='center', fontsize=12)
                            ax_map.axis('off')
                            ax_map.set_title(cond)
                            continue
                        ax_map.imshow(img, cmap='gray')
                        ax_map.axis('off')
                        img_h, img_w = img.shape[:2]
                        cw, ch = img_w / self.data_dict[cond].get_numpy_spectra().shape[1], img_h / self.data_dict[cond].get_numpy_spectra().shape[0]
                        ...
                        x0 = c*cw - cw/2
                        y0 = r*ch - ch/2
                        ax_map.add_patch(Rectangle((x0, y0), cw, ch,
        ...
        btn_prev_col.on_click(lambda _: shift_col(-1))
        btn_next_col.on_click(lambda _: shift_col(1))
        w_row.observe(update_display, names='value')
        w_col.observe(update_display, names='value')
        # w_order_sel.observe(update_display, names='value')
        w_norm.observe(update_display, names='value')
        w_offset.observe(update_display, names='value')
        w_cmap.observe(update_display, names='value')
        w_alpha.observe(update_display, names='value')
        w_lw.observe(update_display, names='value')
        w_logy.observe(update_display, names='value')
        w_show_all_maps.observe(update_display, names='value')

        # --------------------- layout -------------------------
        nav_row = HBox([btn_prev_row, w_row, btn_next_row])
        nav_col = HBox([btn_prev_col, w_col, btn_next_col])
        opts = VBox([HBox([w_norm, w_offset, w_cmap, w_alpha, w_lw, w_logy, w_show_all_maps],
                                    layout=Layout(flex_flow='row wrap', gap='8px'))])
        display(VBox([nav_row, nav_col, opts, out]))

        update_display()

    def interactive_compare_conditions_new(self, start_row=0, start_col=0, ordered_conditions=None):
        """
        Interactive comparison of the same pixel across multiple experimental conditions,
        with ordering, mapping verification, manual pixel correction, and heatmap panel.
        """
        import numpy as np
        import matplotlib.pyplot as plt
        from ipywidgets import (
        ...
        # --------------------- widgets ------------------------
        w_row = BoundedIntText(value=start_row, min=0, max=n_rows - 1, description='Ref Row:', layout=Layout(width='200px'))
        w_col = BoundedIntText(value=start_col, min=0, max=n_cols - 1, description='Ref Col:', layout=Layout(width='200px'))
        ...
        # --- New: Manual correction storage
        # For each condition we create override entries: row_override[cond], col_override[cond]
        manual_override = {cond: {'row': None, 'col': None} for cond in keys_all}

        # Create widget dictionaries for each condition
        override_widgets = {}
        for cond in keys_all:
            override_widgets[cond] = (
                BoundedIntText(value=0, min=0, max=n_rows-1, description=f'{cond} row:', layout=Layout(width='120px')),
                BoundedIntText(value=0, min=0, max=n_cols-1, description=f'{cond} col:', layout=Layout(width='120px'))
            )

        btn_apply_overrides = Button(description='Apply Overrides', button_style='warning')
        ...
        def apply_overrides(_):
            # read override widget values
            for cond in keys_all:
                w_r, w_c = override_widgets[cond]
                manual_override[cond]['row'] = int(w_r.value)
                manual_override[cond]['col'] = int(w_c.value)
            # after applying, redraw
            update_display()
        ...
        def update_display(_=None):
            row, col = w_row.value, w_col.value
            cond_order = list(w_order_sel.value)
            cmap = mpl.cm.get_cmap(w_cmap.value)
            N = len(cond_order)
            colors = cmap(np.linspace(0,1,N))
            ...
            with out:
                out.clear_output(wait=True)
                if w_show_all_maps.value:
                    fig, axes = plt.subplots(1, 3 + N, figsize=(4*(3+N), 8), 
                                            gridspec_kw={'width_ratios':[2]+[1]*(2 + N)})
                else:
                    fig, axes = plt.subplots(1, 3, figsize=(16,8),
                                            gridspec_kw={'width_ratios':[2, 1, 1]})
                    
                # fig, axes = plt.subplots(1, 3, figsize=(16,4), gridspec_kw={'width_ratios':[2,1,1]})
                ax_wf, ax_hm, ax_map = axes[0], axes[1], axes[2] 
                ...
                for i, cond in enumerate(cond_order):
                    ...
                    # determine pixel row/col: either override or mapped
                    if manual_override[cond]['row'] is not None:
                        ...
                        if cond == ref_key:
                            ...
                            mapped = self.data_coordinates.get(cond, {})
                            r, c = mapped.get((row, col), (None, None))
                            if r is None:
                                continue
                ...
                # Waterfall
                for i, (cond, spec) in enumerate(zip(labels, specs)):
                    ax_wf.plot(wls[i], spec + i*off, color=colors[i], lw=w_lw.value,
                            alpha=w_alpha.value, label=cond)
                ax_wf.set_xlabel('Wavelength')
                ax_wf.set_ylabel('Intensity + offset×k')
                if w_logy.value: ax_wf.set_yscale('log')
                ax_wf.legend(title='Condition', fontsize=8)
                ax_wf.set_title(f'Pixel ({row},{col}) across {len(labels)} conditions (with overrides)')

                # Heatmap
                if len(specs) > 1:
                    min_len = min(s.shape[0] for s in specs)
                    hm = np.vstack([s[:min_len] for s in specs])
                    ax_hm.imshow(hm, aspect='auto', origin='upper',
                                cmap=w_cmap.value,
                                extent=[wls[0][0], wls[0][min_len-1], len(hm), 0])
                    ax_hm.set_title('Heatmap (conds × λ)')
                    ax_hm.set_xlabel('Wavelength'); ax_hm.set_ylabel('Condition')
                    ax_hm.set_yticks(np.arange(len(labels)) + 0.5)
                    ax_hm.set_yticklabels(labels)
                    fig.colorbar(mpl.cm.ScalarMappable(cmap=w_cmap.value),
                                ax=ax_hm, fraction=0.046, pad=0.04)
                else:
                    ax_hm.text(0.5,0.5,'Insufficient data', ha='center', va='center')
                    ax_hm.axis('off')

                # Reference map + overrides marker
                if image_ref is not None:
                    ax_map.imshow(image_ref, cmap='gray')
                    ax_map.axis('off')
                    img_h, img_w = image_ref.shape[:2]
                    cell_w, cell_h = img_w / n_cols, img_h / n_rows
                    x0 = col*cell_w - cell_w/2
                    y0 = row*cell_h - cell_h/2
                    ax_map.add_patch(Rectangle((x0, y0), cell_w, cell_h,
                                            linewidth=2, edgecolor='red', facecolor='none'))
                    ax_map.set_title(f'Ref map: {ref_key}')
                else:
                    ax_map.text(0.5,0.5,'No ref-map image', ha='center', va='center')
                    ax_map.axis('off')
        ...
        # --------------------- layout -------------------------
        # build override widgets UI
        overrides_ui = []
        for cond, (w_r, w_c) in override_widgets.items():
            overrides_ui.append(HBox([w_r, w_c], layout=Layout(gap='4px')))
        overrides_box = VBox([Button(description="--- Manual Pixel Overrides ---", disabled=True)] + overrides_ui + [btn_apply_overrides])

        nav_row = HBox([btn_prev_row, w_row, btn_next_row], layout=Layout(gap='5px'))
        nav_col = HBox([btn_prev_col, w_col, btn_next_col], layout=Layout(gap='5px'))
        opts = VBox([w_order_sel, HBox([w_norm, w_offset, w_cmap, w_alpha, w_lw, w_logy, w_show_all_maps],
                                        layout=Layout(flex_flow='row wrap', gap='8px'))])
    ...
    def interactive_compare_conditions_new_new(self, start_row=0, start_col=0, ordered_conditions=None, wlim=None):
        """
    ...
    def __peak_fitting_manual(self, intensity, wavelengths, peak_params, r2_threshold=0.005, fit_bkg = False):
        ...
        #             break
    def interactive_waterfall_new(self, exp_key, start_row=0, start_col=0, end_row=None, end_col=None, wlim=None):
        """
    ...
    def interactive_waterfall(self, exp_key, start_row=0, start_col=0, end_row=None, end_col=None, wlim=None):
        ...
        from ipywidgets import (
            Dropdown, BoundedIntText, IntSlider, FloatText, FloatSlider, Checkbox,
            ToggleButtons, Button, HBox, VBox, Layout, Output, HTML
        )
        from IPython.display import display, FileLink
        from matplotlib.patches import Rectangle
        import matplotlib as mpl
        ...
        btn_export = Button(description='Export CSV/PNG', layout=Layout(width='180px'))
        out = Output(layout=Layout(border='1px solid #ccc', padding='6px'))
        # out_map = Output(layout=Layout(border='1px solid #ddd', padding='6px'))
        ...
        def do_plot(save_paths=False):
            with out:
                ...
                specs, valid = _extract_spectra(coords, data)
                if len(valid) == 0:
                    print("No valid pixels in selection."); return

                specs = specs.astype(float)
                specs = _normalize(specs, w_norm.value)

                # offset heuristic
                off = w_offset.value
                if off == 0.0:
                    off = 0.9 * np.median(np.max(specs, axis=1))
                    if not np.isfinite(off) or off <= 0: off = 1.0
                ...
                # figure: waterfall + (map) + (heatmap optional)
                ncols_fig = 1
                if image_scan is not None: ncols_fig += 1
                if w_heatmap.value: ncols_fig += 1
                fig, axes = plt.subplots(1, ncols_fig, figsize=(15, 4),
                                        gridspec_kw={'width_ratios': [2] + [1]*(ncols_fig-1)})
                if ncols_fig == 1: axes = [axes]
                ax_wf = axes[0]
                ...
                if w_heatmap.value:
                    ...
                    if wlim is not None:
                        lo, hi = sorted(wlim)
                        # Clamp to data bounds (optional but robust)
                        lo = max(lo, float(lam.min()))
                        ...
                        if mask.sum() < 2:
                            ax_hm.clear()
                            ax_hm.text(0.5, 0.5,
                                    'No wavelengths in selected range',
                                    ha='center', va='center', transform=ax_hm.transAxes)
                            ax_hm.axis('off')
                        else:
                            lam_plot = lam[mask]
                            ...
                            ax_hm.set_xlabel('Wavelength')
                            ax_hm.set_ylabel('Spectrum index')
                            ax_hm.set_title(f'Heatmap (λ ∈ [{lo:.2f}, {hi:.2f}])')
                    else:
                        # Full range
                        y0, y1 = (0, hm_specs.shape[0])
                        x0, x1 = (lam.min(), lam.max())
                ...
                #     df_map = pd.DataFrame({'index': np.arange(N),
                #                         'row': [rc[0] for rc in valid],
                #                         'col': [rc[1] for rc in valid]})
                #     state['valid_coords'] = valid
                #     state['mapping_df'] = df_map
                #     # show a compact slice around the highlighted index
                #     lo = max(0, hi-10); hi2 = min(N, hi+11)
                #     display(HTML("<b>Index → (row, col) mapping</b> (showing ~21 around highlight)"))
                #     display(df_map.iloc[lo:hi2].reset_index(drop=True))

                # export
                if save_paths:
                    lam_hdr = [f"{wl:.6f}" for wl in wavelengths]
                    df_specs = pd.DataFrame(specs, columns=lam_hdr)
                    df_map.to_csv(f"waterfall_{exp_dropdown.value}_{mode}_map.csv", index=False)
                    df_specs.to_csv(f"waterfall_{exp_dropdown.value}_{mode}_spectra.csv", index=False)
                    png_path = f"waterfall_{exp_dropdown.value}_{mode}.png"
                    fig2 = plt.figure(figsize=(10, 4))
        ...
        def refresh_on_dataset(change):
            if change['name'] == 'value' and change['new'] != change['old']:
                ...
                for w in (w_c0, w_c1):
                    w.max = n_cols - 1; w.value = min(w.value, n_cols - 1)
                w_highlight.value = 0
                # out.clear_output(True); out_map.clear_output(True)
        ...
        row_sel = HBox([w_r0, w_c0, w_r1, w_c1, w_stride], layout=Layout(gap='8px', flex_flow='row wrap'))
        opts1 = HBox([w_mode, w_order, w_norm, w_offset], layout=Layout(gap='8px', flex_flow='row wrap'))
        opts2 = HBox([w_cmap, w_alpha, w_lw, w_logy, w_heatmap, w_label_every, w_highlight],
                    layout=Layout(gap='8px', flex_flow='row wrap'))
        actions = HBox([btn_plot, btn_export], layout=Layout(gap='10px'))
